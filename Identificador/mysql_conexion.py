...
TRANSACCIONES_VALIDAS = {1, 2, 3, 4, 5}
llamadas_activas = []
...
def manejar_cliente(cliente_socket):
    with cliente_socket:
        try:
            ...
            if not transaccion_valida(datos.get('tipo_transaccion', '')):
                cliente_socket.sendall(json.dumps({"status": "error", "message": "Acción inválida"}).encode())
                return

            # Validar coordenadas solo si aplica
            if datos['tipo_transaccion'] in ["solicitud", "llamada", "finalizacion"]:
                if not coordenadas_validas(datos.get('coordenadas', '')):
                    cliente_socket.sendall(json.dumps({"status": "error", "message": "Coordenadas inválidas"}).encode())
                    return

            # Descifrar número de teléfono
            try:
                if datos.get('telefono', '').startswith("ENC:"):
                    datos['telefono'] = descifrar_aes(datos['telefono'][4:])
            except Exception as e:
                cliente_socket.sendall(json.dumps({"status": "error", "message": "Error al descifrar número de teléfono"}).encode())
                return
            ...
            if not conexion:
                cliente_socket.sendall(json.dumps({"status": "error", "message": "Error de conexión a MySQL"}).encode())
                return

            cursor = conexion.cursor(dictionary=True)
            info = obtener_datos_completos_telefono(cursor, telefono)
            if not info:
                cliente_socket.sendall(json.dumps({"status": "error", "message": "Datos del teléfono no encontrados o inactivos"}).encode())
                cursor.close()
                conexion.close()
                return

            # Añadir datos recuperados
            ...
            print(f"Identificador Tel: {datos.get('identificadorTel')}")
            print(f"Identificador Tarjeta: {datos.get('identificador_tarjeta')}")

            cursor.close()
            conexion.close()

            # Enviar datos al proveedor
            ...
            if datos['tipo_transaccion'] == "llamada":
            ...
            cliente_socket.sendall(json.dumps({"status": "error", "message": str(e)}).encode())
